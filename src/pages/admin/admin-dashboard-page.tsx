...
const tabs: { id: TabId; label: string }[] = [
  ...
  { id: "temoignages", label: "Avis (captures)" },
    ...
    <div className="min-h-screen bg-[var(--color-page)]">
      <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              onClick={() => {
                if (
                  window.confirm(
                    "Réinitialiser toutes les données aux valeurs par défaut du site ?"
                  )
                ) {
                  resetToDefaults();
                  setMsg("Réinitialisé.");
            ...
            <Link
              to="/"
              className="rounded-full bg-[var(--color-cta)] px-4 py-2 text-xs font-bold text-[var(--color-on-cta)]"
            >
              Voir le site
            </Link>
          </div>
        ...
        <p className="mt-2 text-sm text-[var(--color-body)]/80">
          Stockage local au navigateur (localStorage). Exportez un fichier JSON
        ...
        <div className="mt-8 space-y-6">
          ...
          {tab === "temoignages" ? (
            <div className="rounded-xl border border-[var(--color-brand)]/15 bg-[var(--color-muted-green)]/30 p-6 text-sm text-[var(--color-body)]">
              <h2 className="font-display text-lg font-semibold text-[var(--color-ink)]">
                Avis écrits affichés sur le site
              </h2>
              <p className="mt-3 leading-relaxed">
                Les banderoles utilisent les <strong>captures PNG</strong> dans
                le dossier{" "}
                <code className="rounded bg-white/80 px-1.5 py-0.5 font-mono text-xs text-[var(--color-ink)]">
                  public/images/avis_site/
                </code>
                . La liste des fichiers à faire défiler est définie dans{" "}
                <code className="rounded bg-white/80 px-1.5 py-0.5 font-mono text-xs text-[var(--color-ink)]">
                  src/data/avis-site.ts
                </code>{" "}
                (tableau{" "}
                <code className="font-mono text-xs">avisSiteFilenames</code>).
                Ajoutez ou retirez des images dans le dossier, puis mettez à
                jour ce tableau pour refléter l’ordre et les fichiers souhaités.
              </p>
            </div>
          ) : null}
          {tab === "videos" ? (
            <div>
              ...
              <p className="mt-1 text-sm text-[var(--color-body)]/85">
                Un nom de fichier par ligne (`public/images/témoignages/`).
              </p>
              <textarea
                value={vidText}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-3 grid gap-4 sm:grid-cols-2">
          <Field
            ...
            value={state.heroCopy.eyebrow}
            onChange={(v) =>
              setState((s) => ({ ...s, heroCopy: { ...s.heroCopy, eyebrow: v } }))
            }
          />
          <Field
            label="Titre"
            value={state.heroCopy.title}
            onChange={(v) =>
              setState((s) => ({ ...s, heroCopy: { ...s.heroCopy, title: v } }))
            }
          />
          <Field
            label="Sous-titre"
            value={state.heroCopy.subtitle}
            onChange={(v) =>
              setState((s) => ({
                ...s,
                heroCopy: { ...s.heroCopy, subtitle: v },
              }))
            }
          />
          <Field
            label="1er bouton (Prendre rendez-vous → Contact)"
            value={state.heroCopy.ctaPrimary}
            onChange={(v) =>
              setState((s) => ({
                ...s,
                heroCopy: { ...s.heroCopy, ctaPrimary: v },
              }))
            }
          />
          <Field
            label="2e bouton (Programme à distance)"
            value={state.heroCopy.ctaSecondary}
            onChange={(v) =>
              setState((s) => ({
                ...s,
                heroCopy: { ...s.heroCopy, ctaSecondary: v },
              }))
            }
          />
        </div>
        ...
        <div className="mt-3 grid gap-4 sm:grid-cols-2">
          <Field
            ...
            onChange={(v) =>
              setState((s) => ({
                ...s,
                contactInfo: { ...s.contactInfo, phone: v },
              }))
            }
          />
          <Field
            ...
            onChange={(v) =>
              setState((s) => ({
                ...s,
                contactInfo: { ...s.contactInfo, email: v },
              }))
            }
          />
          <Field
            ...
            onChange={(v) =>
              setState((s) => ({
                ...s,
                contactInfo: { ...s.contactInfo, address: v },
              }))
            }
          />
          <Field
            label="Horaires"
            value={state.contactInfo.hours}
            onChange={(v) =>
              setState((s) => ({
                ...s,
                contactInfo: { ...s.contactInfo, hours: v },
              }))
            }
          />
        </div>
      ...
      <div>
        ...
        <div className="mt-3 grid gap-4 sm:grid-cols-2">
          <Field
            ...
            onChange={(v) =>
              setState((s) => ({ ...s, site: { ...s.site, name: v } }))
            }
          />
          <Field
            label="Slogan"
            ...
            onChange={(v) =>
              setState((s) => ({ ...s, site: { ...s.site, tagline: v } }))
            }
          />
          <Field
            label="Domaine (footer)"
            value={state.site.domain}
            onChange={(v) =>
              setState((s) => ({ ...s, site: { ...s.site, domain: v } }))
            }
          />
        </div>
      </div>
      <button
        type="button"
        onClick={() =>
          setMsg("Modifications enregistrées (sauvegarde automatique).")
        }
